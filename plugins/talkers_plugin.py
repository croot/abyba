...
def talkers_get_top(type, source, body, COUNT=10):
        global TALKERS_BD
        room = source[1] # получаем текущую комнату
        MAS = []
        if TALKERS_SORT_OF:
                OUT = u'Статистика\nТОП-участников\nНомер, юзер, сообщ., слов, слов /me\n'
                ...
                if len(MAS) == 0:
                        reply(type, source, u'Походу статистика отсутствует :-)')
                        return
                MAS.sort()
                MAS.reverse()
                ...
                OUT = u'Статистика\nТОП-участников\nНомер, юзер, слов, сообщ., слов /me, с/с\n'
...
def talkers_clear(type, source, body):
        ...
        if user_level(room+'/'+source[2], room)>=20:
                if not T.has_key(room):
                        ...
                        reply(type, source, u'Инициализированно удаление базы данных болтунов ('+str(len(TALKERS_BD[room]))+u' записей), выполните команду повторно чтобы удалить её!')
                        return
                else:
                        if time.time() - T[room]['time'] <= 60:
                                if jid == T[room]['jid']:
                                        ...
                                        reply(type, source, u'База данных очищена, действие занесено в журнал')
                                        return
                                else:
                                        reply(type, source, u'Подтвердить должен тот же кто инициализировал удаление!')
                                        return
                        else:
                                reply(type, source, u'Таймаут 60 секунд!')
                                del T[room]
                                return
        else:
                reply(type, source, u'Мало прав!')
                return
...
def talkers_info(type, source, parameters):
        ...
        #OUT+= u'Plagin is '+TALKERS_BOT_VERSION['COMMENT']
        reply(type, source, OUT)
...
def talkers_get(type, source, parameters):
        ...
        OUT = u'Статистика\nЮзер, Сообщ., Слов, /me, с/с\n%NICK%: %MESSAGES% %WORDS% %MES% %SS%'
        if not parameters:
                if TALKERS_BD[room].has_key(jid):
                        OUT = OUT.replace(u'%NICK%', TALKERS_BD[room][jid]['nick']).replace(u'%MESSAGES%', str(TALKERS_BD[room][jid.lower()][u'messages'])).replace(u'%WORDS%', str(TALKERS_BD[room][jid.lower()]['words'])).replace(u'%MES%', str(TALKERS_BD[room][jid.lower()]['mes'])).replace(u'%SS%', str( round((TALKERS_BD[room][jid.lower()]['words'] * 1.0) / (TALKERS_BD[room][jid.lower()][u'messages'] * 1.0), 1) ))
                        reply(type, source, OUT)
                else:
                        reply(type, source, u'Похожу на тебя статистики нет')
        else:
                if parameters in [u'инфо', u'чисть', u'топ']:
                        if parameters == u'топ':
                                talkers_get_top(type, source, parameters)
                        elif parameters == u'чисть':
                                talkers_clear(type, source, parameters)
                        elif parameters == u'инфо':
                                talkers_info(type, source, parameters)
                        else:
                                reply(type, source, u'Параметр опознан как внутренняя команда, но сейчас ее пока нет')
                        ...
                        if TALKERS_BD[room].has_key( get_true_jid(room+'/'+parameters).lower() ):
                                ...
                                reply(type, source, u'Походу на него статистики нет')
                        ...
                        reply(type, source, u'Гг, параметр не опознан')


def talkers_stat(raw, type, source, body):
        global TALKERS_BD
        ...
        jid = get_true_jid(source[1]+'/'+source[2]) # получаем жид
        nick = source[2] # получаем ник
        jidbot = TALKERS_BOT_VERSION['JID'] # получаем реальный жид бота
        nickbot = get_bot_nick(source[1]) # получаем ник бота в данной комнате

        if (jid == jidbot) | (jid in TALKERS_NOT_LOG) | (jid == room) | (type == 'private'):
                return

        if not TALKERS_BD[room].has_key( jid ):
                TALKERS_BD[room][ jid ] = {'nick':nick, 'words':0, 'messages':0, 'mes':0, 'words_1_letter':0, 'words_2-3_letter':0, 'words_4-6_letter':0, 'words_up6_letter':0}

        
        TALKERS_BD[room][ jid ]['messages'] += 1
        if body.count('/me') > 0:
                TALKERS_BD[room][ jid ]['mes'] += 1
        WORDS = body.split(' ')
        TALKERS_BD[room][ jid ]['words'] += len( WORDS )

        # [BEGIN STATS OF LENGTH WORDS] Данный кусок считает статистику по длинне слов. Сейчас толком не реализовано.
        if TALKERS_LOG_ON_LENGTH:
                for W in WORDS:
                        if len(W) == 1:
                                TALKERS_BD[room][ jid ]['words_1_letter'] += 1
                        elif (len(W) > 1) & (len(W) < 4):
                                TALKERS_BD[room][ jid ]['words_2-3_letter'] += 1
                        elif (len(W) > 3) & (len(W) < 7):
                                TALKERS_BD[room][ jid ]['words_4-6_letter'] += 1      
                        elif (len(W) > 6):
                                TALKERS_BD[room][ jid ]['words_up6_letter'] += 1
        # [END STATS OF LENGTH WORDS]
        
        # [BEGIN SAVE STATS] Данный кусок сохраняет статистику по программе SAVE_COUNT
...
register_command_handler(talkers_get, 'болтуны', [], 0, 'Показывает твою статистику в этой комнате', ' [ключ/жид/ник]', ['болтуны топ', 'болтуны Gigabyte', 'болтуны jid@jabber.ru'])
